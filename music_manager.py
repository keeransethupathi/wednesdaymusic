...
def get_artist_top_songs(artist_id, limit=50):
    """
    Retrieve top songs for an artist using their public token.
    Returns a list of parsed track dictionaries.
    """
    if not artist_id:
        return []
    ...
    try:
        url = f"https://www.jiosaavn.com/api.php?__call=webapi.get&type=artist&token={artist_id}&_format=json&cc=in&n_song={limit}&n_album=0"
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        }
        res = requests.get(url, headers=headers, timeout=15)
        ...
        data = res.json()
        top_songs_dict = data.get("topSongs", {})
        raw_songs = top_songs_dict.get("songs", []) if isinstance(top_songs_dict, dict) else []
        parsed_songs = []
        ...
        print(f"[Music Manager] Error loading artist top songs for {artist_id}: {e}")
        return []

def get_ilaiyaraaja_specials():
    """
    Fetch and combine top Tamil songs of Ilaiyaraaja from multiple sources
    to compile a unique list of exactly 50 legendary tracks.
    """
    sources = [
        ("playlist", "i30y05XZsGY_"),          # Let's Play - Ilaiyaraaja (Tamil)
        ("playlist", "2kqHw3QtZkILtNrz-hs7eg__"), # Ilaiyaraaja Tamil Hits - Carvaan Select
        ("artist", "ciCHe-IFG1w_")             # Artist profile top songs (filtered)
    ]
    ...
    for stype, sid in sources:
        try:
            if stype == "playlist":
                # Fetch up to 100 songs from the playlist
                tracks = get_playlist_tracks(sid, limit=100)
            else:
                # Fetch up to 70 songs from the artist profile
                tracks = get_artist_top_songs(sid, limit=70)
                
            for t in tracks:
                tid = t.get("track_id")
                # Deduplicate and ensure it has a valid stream URL
                if tid and tid not in unique_songs and t.get("stream_url"):
                    unique_songs[tid] = t
        except Exception as e:
            print(f"[Music Manager] Error loading source {sid} for Ilaiyaraaja Specials: {e}")
            
    # Convert to list
    all_songs = list(unique_songs.values())
